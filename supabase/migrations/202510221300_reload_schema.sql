-- Reload PostgREST schema to expose new functions
select pg_notify('postgrest', 'reload schema');

-- Ensure function is accessible without parameters
...
-- Grant execute to authenticated users (anon and authenticated roles)
