...
class _EditOrderScreenState extends ConsumerState<EditOrderScreen> {
  ...
  Future<void> _loadOrderImages() async {
    try {
      final records = await ref
          ...
          .getOrderImages(widget.order.id);
      if (!mounted) return;
      setState(() {
        _persistedRecords
          ..clear()
    ...
    if (widget.order.clientId != null) {
      try {
        final client = await ref
            ...
            .getClientById(widget.order.clientId!);
        if (mounted) {
  ...
  Future<void> _updateOrder() async {
    ...
    try {
      ...
      await ref
          ...
          .updateOrder(widget.order.id, updatedOrder);
      ...
      if (_localImagesCount > 0) {
        try {
          ...
          final existingRecords = await ordersSvc.getOrderImages(
            widget.order.id,
          );
          ...
          for (var i = 0; i < existingRecords.length; i++) {
            ...
            final title = i < _persistedTitles.length
                ? (_persistedTitles[i].trim().isEmpty
                      ? null
                      : _persistedTitles[i].trim())
                : rec.title;
            final desc = i < _persistedDescs.length
                ? (_persistedDescs[i].trim().isEmpty
                      ? null
                      : _persistedDescs[i].trim())
                : rec.description;
            updatedExisting.add(
              OrderImageRecord(
                url: rec.url,
          ...
          for (var i = 0; i < uploadedUrls.length; i++) {
            final t = i < _newTitles.length
                ? (_newTitles[i].trim().isEmpty ? null : _newTitles[i].trim())
                : null;
            final d = i < _newDescs.length
                ? (_newDescs[i].trim().isEmpty ? null : _newDescs[i].trim())
                : null;
            newRecords.add(
              OrderImageRecord(
                url: uploadedUrls[i],
                position: updatedExisting.length + i,
                title: t,
                description: d,
              ),
            );
        ...
        try {
          ...
          await ordersSvc.deleteImagesForOrder(widget.order.id);
          await ordersSvc.addOrderImagesWithMeta(
            widget.order.id,
            updatedExisting,
          );
          await _loadOrderImages();
  ...
  Widget build(BuildContext context) {
    ...
    final bottomPadding =
        16.0 + bottomSafeArea + (keyboardInset > 0 ? keyboardInset : 0);
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText:
                      'Descreva detalhadamente o serviço a ser realizado...',
                  border: OutlineInputBorder(),
                ),
              ),
              const SizedBox(height: 16),
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      ...
                                      child: TextField(
                                        ...
                                        controller: TextEditingController(
                                          text: _persistedTitles[index],
                                        ),
                                        onChanged: (v) =>
                                            _persistedTitles[index] = v,
                                      ),
                                    ),
                                    ...
                                    SizedBox(
                                      ...
                                      child: TextField(
                                        ...
                                        style: const TextStyle(fontSize: 12),
                                        controller: TextEditingController(
                                          text: _persistedDescs[index],
                                        ),
                                        onChanged: (v) =>
                                            _persistedDescs[index] = v,
                                      ),
                                    ),
                                  ],
                                ),
