...
class ImageUploadService {
  ...
  // Suporte Web: múltiplas imagens em bytes
  Future<List<String>> uploadOrderImagesBytes(List<Uint8List> imagesBytes, String orderId, {List<String?>? originalNames}) async {
    final List<String> uploadedUrls = [];
    for (var i = 0; i < imagesBytes.length; i++) {
      try {
        final url = await uploadOrderImageBytes(imagesBytes[i], orderId, originalName: originalNames != null && i < originalNames.length ? originalNames[i] : null);
        uploadedUrls.add(url);
      } catch (error) {
        // Log e continua
        // ignore: avoid_print
  ...
  // Suporte Web: upload a partir de bytes (Uint8List)
  Future<String> uploadOrderImageBytes(Uint8List bytes, String orderId, {String? originalName}) async {
    try {
      // Validar tamanho
      final fileSize = bytes.lengthInBytes;
      ...
      // Inferir extensão a partir do nome original (se existir) ou usar .jpg como padrão
      String extension = '.jpg';
      if (originalName != null && originalName.contains('.')) {
        extension = originalName.substring(originalName.lastIndexOf('.'));
      }
      final extNoDot = extension.replaceAll('.', '').toLowerCase();
      if (!SupabaseConfig.allowedImageTypes.contains(extNoDot)) {
        // Tentar fallback para jpg
        extension = '.jpg';
      }
      ...
      // Upload binário (método compatível com Web)
      await _supabase.storage
          .from(SupabaseConfig.imagesBucket)
          .uploadBinary(
            filePath,
            bytes,
            fileOptions: FileOptions(contentType: 'image/${extension.replaceAll('.', '')}'),
          );
